...
@section('title') Edit Feedback @endsection
@section('content')
@section('stylesheet')  
...
<form class="form-horizontal" role="form" action="{{ route('post.dashboard.feedback.edit', ['id'=>$id])}}" method="post" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group">
        ...
        <div class="col-xs-9">
            <textarea name="content" id="content" rows="9" class="col-xs-9 col-sm-5">{{ old('content', isset($data) ? $data['content'] : '' ) }}</textarea>
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-9">
            @if($data['thumbnail'] != '')
                <img class="react__shape__ab" src="{{asset($data['thumbnail'])}}">
            @else
